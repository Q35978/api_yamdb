...
class UserRegisterationSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def validate(self, data):
        ...
        validation_error = ''
        if error_username:
            validation_error += error_username
        if found_by_username:
            validation_error += (' Пользователь с таким именем '
                                 'пользователя уже существует!')
        if found_by_email:
            validation_error += (' Пользователь с таким адресом эл. почты '
                                 'уже существует!')
        if validation_error:
            raise serializers.ValidationError(validation_error.strip())
        return data
